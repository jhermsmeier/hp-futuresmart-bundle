...
class Bundle extends File {
  ...
  async readHeader() {
    ...
    this.packageCount = buffer.readUInt32LE( offset ); offset += 4
    ...
    this.unknown4 = buffer.readUInt8( offset ); offset += 1
    this.unknown5 = buffer.readUInt32LE( offset ); offset += 4
    this.unknown6 = buffer.readUInt32LE( offset ); offset += 4
  ...
  async readEntries( position = this.headerSize ) {
    ...
    const length = Bundle.ENTRY_SIZE * this.packageCount
  ...
  async extractPackages( dirname ) {
    ...
    for( let entry of this.packages ) {
      ...
      const filename = path.join( dirname, `${ pkg.name }.ipkg` )
